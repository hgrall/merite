...
import { NoeudChatImmutable, creerNoeudChatImmutable } from '../commun/noeudChat';
import { creerMessageCommunication, TypeMessageChat, FormatMessageChat, EtiquetteMessageChat, MessageChat } from '../commun/messageChat';
import { FormatErreurChat, EtiquetteErreurChat, ErreurChat, creerErreurChat } from '../commun/erreurChat';
import {
	FormatConfigurationChat,
	EtiquetteConfigurationChat,
	ConfigurationChat,
	creerConfigurationChat,
...
class CorpsBrut extends React.Component<ProprietesCorps, EtatCorps> {
	...
	render(): JSX.Element {
		switch (this.state.etatInterface) {
			...
			case EtatInterfaceChat.INITIAL:
				return <h1>Connexion au serveur pour l'initialisation</h1>;
			case EtatInterfaceChat.ERRONE:
				return (
					<div>
						<h1>Fin de l'application après l'erreur suivante : </h1>
						<div style={{ color: TEXTE_ERREUR }}>{this.messageErreur}</div>
					</div>
				);
		}
	}
	...
	componentDidMount(): void {
		...
		this.canal.enregistrerTraitementConfigurationRecue((c: FormatConfigurationChat) => {
			...
			this.individusObjets = creerTableIdentificationImmutable(
				'sommet',
				creerTableImmutable(this.noeud.val().voisins)
					.application(s => {
						let c = suite.courant();
						return {
							ID: s.ID,
