...
import {
  hote, port2,
  NoeudJeu1Immutable, creerNoeudJeu1Immutable,
  creerMessageCommunication,
  TypeMessageJeu1, FormatMessageJeu1, EtiquetteMessageJeu1, MessageJeu1,
  FormatConfigurationJeu1, EtiquetteConfigurationJeu1,
  ConfigurationJeu1, creerConfigurationJeu1,
...
class CorpsBrut extends React.Component<ProprietesCorps, EtatCorps> {
  ...
  private noeud: NoeudJeu1Immutable;
  ...
  envoyerMessage(m: Message, d: DateImmutable) {
    ...
    if (m.destinataire.ID.val === ID_TOUS) {
      ...
      this.individusObjets.iterer((c, v) => {
        let msg: MessageJeu1 = creerMessageCommunication(m.ID, m.emetteur.ID, v.ID, m.contenu, d.val());
    ...
    let msg: MessageJeu1 = creerMessageCommunication(m.ID, m.emetteur.ID, m.destinataire.ID, m.contenu, d.val());
  ...
  componentDidMount(): void {
    ...
    this.canal.enregistrerTraitementMessageRecu((m: FormatMessageJeu1) => {
      ...
      /* Message en transit */
      if (m.type === TypeMessageJeu1.TRANSIT) {
    ...
    this.canal.enregistrerTraitementConfigurationRecue((c: FormatConfigurationJeu1) => {
      ...
      this.noeud = creerNoeudJeu1Immutable(decomposerConfiguration(config));
      this.individuSujet = {
